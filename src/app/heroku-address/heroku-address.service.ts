...
export class HerokuAddressService {
  ...
  getColonies(id_mun:number): Observable<Colony[]> {
    return this.httpClient.get<Colony[]>(this.endpoint + '/municipalities/' + id_mun + '/colonies')
    .pipe(
      retry(1)
    )
  }
  ...
  getMunicipalities(state_code:number): Observable<Municipality[]> {
    return this.httpClient.get<Municipality[]>(this.endpoint + '/states/' + state_code + '/municipalities')
    .pipe(
      retry(1)
    ) 
  }
  ...
  getColoniesFromZip(zip:string): Observable<Colony[]> {
    return this.httpClient.get<Colony[]>(this.endpoint + '/zip/' + zip + '/colonies')
    .pipe(
      retry(1)
    )
  }
